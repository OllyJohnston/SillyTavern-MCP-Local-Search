...
export class ChallengeHandler {
  ...
  public static async handleBingChallenge(page: Page): Promise<boolean> {
    ...
    console.log(`[ChallengeHandler] BING: Anti-bot challenge detected. Attempting bypass with ${CAPTCHA_POLLING_ATTEMPTS}s polling...`);
    ...
    try {
      ...
      for (let attempt = 1; attempt <= CAPTCHA_POLLING_ATTEMPTS; attempt++) {
        ...
        if (attempt % 4 === 0) console.log(`[ChallengeHandler] BING: Still polling for captcha elements... (${attempt}/${CAPTCHA_POLLING_ATTEMPTS})`);
        await page.waitForTimeout(CAPTCHA_POLLING_INTERVAL);
      }

      if (!box || !targetFrame) {
        console.log(`[ChallengeHandler] BING: Could not find captcha interaction point after ${CAPTCHA_POLLING_ATTEMPTS}s.`);
        return false;
      }
